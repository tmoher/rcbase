...
function step_feet() { 
	...
	for (var i=0; i<FOOTSTEPS.length; i++) {
		var f = FOOTSTEPS[i];
		ctx_footsteps.globalAlpha = f.alpha/2;
...
function initialize_render() {
	...
	$('#section').html(top.roomcast_section().printName);
...
function preloadImages(callback) { 
	var bg = 'background-' + top.cluster_name;

	var images = [bg,'left_foot','right_foot','eco1','eco2','eco3','eco4','eco5','mfw'];
    var img;
    ...
    for (var i = 0; i < images.length; i++) {
        ...
        switch (images[i]) {
        	case 'left_foot': LEFT_FOOT = img; break;
        	case 'right_foot': RIGHT_FOOT = img; break;
        	case 'eco1': ECO1 = img; break;
        	case 'eco2': ECO2 = img; break;
        	case 'eco3': ECO3 = img; break;
        	case 'eco4': ECO4 = img; break;
        	case 'eco5': ECO5 = img; break;
        	case 'mfw': MFW = img; break;
...
var ECO1,ECO2,ECO3,ECO4,ECO5,MFW;
...
$(window).on("mousedown", function (event){
	if (event.pageY > CANVAS_HEIGHT) return;
 	top.zap_public_resources(); 
	AT = -1; 
	top.PUBLIC_DEVICE_NAME = ''; 
	top.PUBLIC_PORTAL = '';
	nutella.net.publish('updated_public_resources',top.individual_name().printName);
	// return;
...
function render_publics() {
	ctx_publics.clearRect(0,0,CANVAS_WIDTH,CANVAS_HEIGHT);

	for (var i=0; i<publics.length; i++) {
		if (publics[i].img_name == '') continue;
		...
		if (i == 5) img = MFW;
  		// var img = new Image();
		// img.src = publics[i].img_name; //"eco" + (i+1) + ".png";
		img.width = .1 * CANVAS_HEIGHT;
		// hack
		if (publics[i].img_name == 'mfw.png') img.width = .08 * CANVAS_WIDTH;
		img.height = .1 * CANVAS_HEIGHT;
		// alert(img.width + ' ' + img.height);
		// alert(publics[i].location.x*CANVAS_WIDTH - img.width/2);

		// ctx_map.drawImage(img,0,0,800,664,publics[i].location.x*CANVAS_WIDTH, publics[i].location.y*CANVAS_HEIGHT
		// 	,30,30);		
		ctx_publics.drawImage(img,publics[i].location.x*CANVAS_WIDTH - img.width/2,publics[i].location.y*CANVAS_HEIGHT - img.height/2,img.width,img.height);
		// if (publics[i].portal == 'table') return; //hackadelic
		// ctx_map.fillStyle = "maroon";
		// // ctx_map.font = "200% Arial";
		// ctx_map.font = Math.floor(CANVAS_HEIGHT/20) + 'px Arial'
		// ctx_map.shadowOffsetX = 3;
		// ctx_map.shadowOffsetY = 3;
		// ctx_map.shadowColor = "lightgray";
		// ctx_map.shadowBlur = 2;
		// ctx_map.textAlign = 'center';
		// var namestring = publics[i].name; 
		// if (publics[i].portal == 'claims') namestring = ''; //hack
...
function update_resources() { 
    ...
    for (var i=0; i<publics.length; i++){
 	...
 	if (min_d > PUBLIC_PROXIMITY_THRESHHOLD) {
 		// top.zap_public_resources(); 
 		// AT = -1; 
 		// top.PUBLIC_DEVICE_NAME = ''; 
 		// top.PUBLIC_PORTAL = '';
