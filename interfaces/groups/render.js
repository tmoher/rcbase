...
function update_locations () {
	...
	for (var i=0; i<participants.length; i++) {
		...
		if (current.x != destination.x || current.y != destination.y) {
			...
			if (d < UNIT_MOVE_PCT) {
				current.x = destination.x; current.y = destination.y;
				if (p.name == top.roomcast_name().printName) ARRIVAL = true;
				update_chat();
			} else {
				current.x = current.x + (UNIT_MOVE_PCT / d) * (destination.x - current.x);
				current.y = current.y + (UNIT_MOVE_PCT / d) * (destination.y - current.y);
...
function initialize_render() {
	...
	var my_name = top.roomcast_name().printName;
	nutella.net.publish ('participant_leaving',my_name);
	var participant  = {        
...
nutella.net.subscribe('updated_participant',function(participant){ 
	if (participant.name == top.roomcast_name().printName)return; 
	for (var i=0; i<participants.length; i++){
...
nutella.net.subscribe('participant_left',function(name){
	if (name == top.roomcast_name().printName)return; 
	for (var i=0; i<participants.length; i++){
...
function render_publics() {
	for (var i=0; i<publics.length; i++) {
		...
		ctx_map.textAlign = 'center';
		ctx_map.fillText(publics[i].portal + ' ' + publics[i].name, publics[i].location.x*CANVAS_WIDTH, publics[i].location.y*CANVAS_HEIGHT); 
	}
}
...
function update_resources() { 
    var my_loc ={x:0,y:0};
    for (var i=0; i<participants.length; i++){
      if (participants[i].name == top.roomcast_name().printName){
